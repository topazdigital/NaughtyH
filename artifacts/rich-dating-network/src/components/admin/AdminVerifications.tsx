    ...
    <div className="space-y-4">
          ...
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{
                ...
                background: filter === f ? "#6B1FA2" : "#1e293b",
      ...
      {loading ? (
        <div style={{ display: "flex", justifyContent: "center", padding: "3rem 0" }}>
          <div style={{ width: 32, height: 32, border: "2px solid #6B1FA2", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      ) : users.length === 0 ? (
