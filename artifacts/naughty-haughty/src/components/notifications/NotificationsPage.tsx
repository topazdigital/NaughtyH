...
const ICON_BG_LIGHT: Record<string, string> = {
  like: 'rgba(236,72,153,0.1)',
  match: 'rgba(239,68,68,0.1)',
  message: 'rgba(59,130,246,0.1)',
  visit: 'rgba(107,31,162,0.08)',
  gift: 'rgba(251,191,36,0.1)',
  premium: 'rgba(251,191,36,0.1)',
  superlike: 'rgba(59,130,246,0.1)',
}
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            {notifications.length > 0 && (
              <p className="text-gray-500 text-xs mt-0.5">{notifications.filter(n => n.read === 0).length} unread</p>
            )}
          </div>
        </div>
        ...
        {notifications.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-gray-500 text-sm mb-6">You have no notifications yet. Start connecting with people!</p>
            <Link href="/discover"
          ...
          <div className="space-y-1.5">
            {notifications.map((n: any) => (
              <Link key={n.id} href={n.link || '#'}
                ...
                style={{
                  background: n.read === 0 ? 'rgba(107,31,162,0.06)' : '#ffffff',
                  border: n.read === 0 ? '1px solid rgba(107,31,162,0.2)' : '1px solid rgba(0,0,0,0.06)',
                }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = n.read === 0 ? 'rgba(107,31,162,0.1)' : '#f9fafb'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = n.read === 0 ? 'rgba(107,31,162,0.06)' : '#ffffff'}>
                <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0"
                  style={{ background: ICON_BG_LIGHT[n.type] || 'rgba(107,31,162,0.06)', border: '1px solid rgba(0,0,0,0.05)' }}>
                  {ICONS[n.type] || <Bell size={18} className="text-gray-400" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm leading-snug ${n.read === 0 ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>{n.message}</p>
